const SYSTEM_PROMPT = `당신은 공식 회의록 작성자입니다. 제공된 회의 전사문만 근거로 한국어 회의록을 작성하세요.
...
type Payload = { provider?: "openai" | "gemini"; model?: string; apiKey?: string; transcript?: string };
...
export async function POST(request: Request) {
  ...
  const { provider, model, apiKey, transcript } = body;
  ...
  try {
    let markdown = "";
    if (provider === "openai") {
      const response = await fetch("https://api.openai.com/v1/responses", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` }, body: JSON.stringify({ model: model || "gpt-5.6-luna", input: [{ role: "system", content: [{ type: "input_text", text: SYSTEM_PROMPT }] }, { role: "user", content: [{ type: "input_text", text: transcript }] }] }) });
      ...
      const response = await fetch(endpoint, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ systemInstruction: { parts: [{ text: SYSTEM_PROMPT }] }, contents: [{ role: "user", parts: [{ text: transcript }] }] }) });
